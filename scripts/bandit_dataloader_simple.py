...
# ============================================================
# Main builder: full 129-action universe every time
# ============================================================

def build_bandit_dataset_simple(
    ...
    *,
    ...
    utility_second_weight: float = 0.35,
    utility_third_weight: float = 0.10,
    ...
    """
    Full 129-arm builder:
      - no triggering
      - all subsets of kept nodes of size 1..max_subset_size are available every time
    ...
    # --------------------------------------------------------
    # 2) Sensor coordinates and normalization
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    # 3) Extract raw per-time arrays from seq
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    # 5) Precompute full subset universe and subset-static features
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    # 6) Build per-node temporal features
    # --------------------------------------------------------
    ...
    for t in range(T):
        ...
        for m in range(M):
            ...
            term1 = np.exp(-(d1 ** 2) / (2 * rho ** 2))
            term2 = utility_second_weight * np.exp(-(d2 ** 2) / (2 * rho ** 2)) if np.isfinite(d2) else 0.0
            term3 = utility_third_weight * np.exp(-(d3 ** 2) / (2 * rho ** 2)) if np.isfinite(d3) else 0.0
