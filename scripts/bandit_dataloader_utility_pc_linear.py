...
def _clipped_linear_term(d: float, radius: float) -> float:
    return float(max(0.0, 1.0 - d / radius))


def build_bandit_dataset_utility_pc_linear(
    ...
    *,
    ...
    """
    Full 129-arm builder with piecewise-clipped linear utility:

        u(S) = max(0, 1 - d1/R)
             + w2 * max(0, 1 - d2/R)
             + w3 * max(0, 1 - d3/R)

    where R = utility_radius.
    """
    ...
    vprint("[4/8] Choosing clipped-linear utility radius...")
    if utility_radius is None:
        nearest_dists = np.min(dist_mat, axis=1)
        nearest_dists = nearest_dists[np.isfinite(nearest_dists)]
        if len(nearest_dists) == 0:
            utility_radius = 1.0
        else:
            base = float(np.median(nearest_dists))
            utility_radius = max(2.0 * base, 1.0)
    vprint(f"      utility_radius = {utility_radius:.4f}")
    ...
    for t in range(T):
        ...
        for m in range(M):
            ...
            term1 = _clipped_linear_term(d1, utility_radius)
            term2 = utility_second_weight * _clipped_linear_term(d2, utility_radius) if np.isfinite(d2) else 0.0
            term3 = utility_third_weight * _clipped_linear_term(d3, utility_radius) if np.isfinite(d3) else 0.0
